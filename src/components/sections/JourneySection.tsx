      ...
      <div className="container mx-auto px-6 max-w-6xl">
        ...
        >
          ...
          <p className="text-muted-foreground mt-4 max-w-4xl mx-auto leading-relaxed text-left md:text-center">
            Nama saya Syarif Hasyim, lahir pada 28 September 2003. Saya tumbuh
            dalam hangatnya keluarga yang sederhana namun penuh warna. Ayah
            saya, Zaiful, seorang guru SD yang humoris, dan Ibu saya, Siti
            Hajar, adalah pelabuhan cinta terdalam saya. Saya memiliki dua adik
            perempuan yang melengkapi kebahagiaan kami. Inilah sepenggal kisah
            ...
            {journeyData.map((item, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`flex items-center justify-center w-full h-full rounded-full ${item.bg}/20 ${item.color}`}
                    >
                      <item.icon size={18} />
                    </div>
                  </motion.div>
                  ...
                  >
                    <div className="bg-background/60 backdrop-blur-md border border-border/50 p-6 sm:p-8 rounded-3xl shadow-xl hover:shadow-2xl hover:border-primary/30 transition-all duration-300 relative group">
                      ...
                      <div
                        className={`absolute -top-10 -right-10 w-32 h-32 rounded-full blur-[50px] opacity-0 group-hover:opacity-20 transition-opacity duration-500 pointer-events-none ${item.bg}`}
                      />

                      <span
                        className={`inline-block px-3 py-1 rounded-full text-xs font-bold mb-4 bg-muted ${item.color}`}
                      >
                        {item.date}
                      </span>
